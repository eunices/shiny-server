...
<p style="text-align: center;">This document is best viewed on a computer and is not optimised for mobile viewing.</p>
...
Total number of cases to date: `r df_cases_day[nrow(df_cases_day)]$total`
...
```{r} 
max_load = (9400*0.25) + 300         # hospital capacity
...
date_bvh = as.Date("2020-04-11")     # date of opening of bvh
...
df_cases_day$max_load = ifelse(df_cases_day$date < date_bvh, max_load, max_load + beds_bvh)
...
From [24 Mar 2020](https://go.gov.sg/measures24mar), those who still test positive but display no symptoms will no longer be hospitalised but moved to isolation facilities. Approximately 60% of cases are hospitalised.
...
renderPlot({
  ggplot(data=df_cases_day[date<=input$range_date_case[2] & date>=input$range_date_case[1]], 
    ...
    geom_vline(xintercept=date_no_hosp, size=.5, col=cbp1[2], linetype="dotted") +
...
According to [a Straits Times article]((https://www.straitstimes.com/singapore/health/coronavirus-experts-say-big-jump-in-infections-is-stretching-the-healthcare-system)), there are approximately 9,400 public hospital beds. Thus if we assume 25% bed space availability, there would be `9400*0.25 + 300 =` `r max_load` beds available with the addition of 300 beds from the NCID. Note that this is a very rough estimate and that hospitals could possibly expand in capacity where needed (e.g. in the NCID can be expanded to [>500 beds](https://www.ncid.sg/Facilities-Services/Pages/default.aspx)).

However, private hospital, which have about 1,600 beds according to the [article](https://www.straitstimes.com/singapore/health/coronavirus-experts-say-big-jump-in-infections-is-stretching-the-healthcare-system), were not included in the hospital beds maximum load, as they have not been housing many patients at the moment.
...
As of 17 Apr 2020, the hospitals seem to be filling up fast, and Singapore would probably need to change its strategy soon, not to admit [milder Covid-19 cases](https://www.straitstimes.com/singapore/health/coronavirus-experts-say-big-jump-in-infections-is-stretching-the-healthcare-system) or to transfer milder cases out into isolation facilities.
...
renderPlot({
  ...
  ggplot(data=df_cases_day[date<=input$range_date_pl[2] & date>=input$range_date_pl[1]], 
    ...
    geom_vline(xintercept=date_no_hosp, size=.5, col=cbp1[2], linetype="dotted") +
    geom_vline(xintercept=date_bvh, size=.5, col=cbp1[2], linetype="dotted") +
...
```{r}
prop_admitted20 = .2
show_serious = df_cases_day[,c('date', 'case_load')]
show_serious$admitted20 = df_cases_day$case_load
show_serious$admitted20 = ceiling(show_serious$admitted20 * prop_admitted20)
show_serious$proportion20 = round(show_serious$admitted/max_load*100, 1)

increased_yesterday_prop = 
  show_serious[dim(show_serious)[1]]$proportion20-show_serious[(dim(show_serious)[1]-1)]$proportion20
increased_yesterday = 
  show_serious[dim(show_serious)[1]]$case_load-show_serious[(dim(show_serious)[1]-1)]$case_load
increased_yesterday_serious = increased_yesterday*prop_admitted20
```

Assuming admitting only serious cases (`r prop_admitted20*100`% of active cases), on `r format(show_serious[dim(show_serious)[1]]$date, "%d %b %Y")` with `r show_serious[dim(show_serious)[1]]$case_load` cases, the hospitals are filled by `r show_serious[dim(show_serious)[1]]$proportion20`% and is increasing at a rate of `r increased_yesterday_prop`% (or `r increased_yesterday` cases with `r round(increased_yesterday_serious, 0)` serious cases) from yesterday.


*Datasets below were not updated from 20 Apr onwards*

### Not updated

#### New cases through time

The first case of Covid-19 was detected in Singapore on `r format(min_date, "%d %b %Y")`, after the first case was detected in 17 Nov 2019 in China.

Key measures and events are marked in the charts below. Just at a glance, the border control measures and events (category 4) seem to be effacious as the number of imported cases dropped 31 Mar onwards.
...
renderPlot({
  ...
  data = merge(data, labels[grepl("event", type)], by.x=c("confirmed.at.date", "infection.type"), 
               by.y=c("date", "infection.type"), all.x=T, all.y=T)

  data = data[confirmed.at.date >= min_date & confirmed.at.date <= max_date]
  data[is.na(data$R)]$R = 0
  data[is.na(data$D)]$D = 0
  data[is.na(data$N)]$N = 0

  add_date = 3
  annotation_y = 50

  ggplot(data=data, aes(x=confirmed.at.date, y=N, fill=infection.type, label=no)) +

    # implement gradient alpha if have time
    annotate(geom="rect", xmin=min(data$confirmed.at.date-add_date),
             xmax=max(data$confirmed.at.date)+add_date, 
             ymin=0, ymax=Inf, fill="skyblue", alpha=0.2) +
    annotate("text", x = min(data$confirmed.at.date), y=annotation_y, label="Infected", hjust=0) +
    
    annotate(geom="rect", xmin=min(data$confirmed.at.date-add_date), 
             xmax=max(data$confirmed.at.date)+add_date, 
             ymin=-Inf, ymax=0, fill="#dbffbf", alpha=0.5) +
    annotate("text", x = min(data$confirmed.at.date), y=-annotation_y, label="Removed", hjust=0) +
...
show_measures[,1] = c(
  ...
  "Measures to handle the spread within dormitories")
...
renderPlot({
  # Plot 1
  data1 = df[infection.source=="Imported case", list(N=.N),by=c("confirmed.at.date")]
  data1 = merge(data1, labels[grepl("2", related)], by.x=c("confirmed.at.date"), by.y=c("date"), all.x=T, all.y=T)


  plt1 = ggplot(data=data1, aes(x=confirmed.at.date, y=N, label=no)) +
    geom_bar(position='stack', stat="identity", fill=cbp1[5]) + 
    ...
    scale_y_continuous(breaks=breaks_f) +
    scale_x_date(date_breaks = "week", date_minor_breaks = "1 day", date_labels = "%d-%b",
                 limits=c(min_date-1, max_date+2)) +

    geom_label_repel(nudge_y=20, direction="x", angle=90, vjust=0, force=5, 
                     fontface='bold', color="black", size=3, 
                     segment.size=0.5, segment.color='black', fill='white',
                     box.padding = unit(0.4, 'lines'), 
                     arrow = arrow(length = unit(1, "mm")),
                     point.padding = 0, max.iter=3000, show.legend=F) 
  
  df2 = merge(df, lp_clust[, c("cluster", "foreign.workers")], by.x="cluster", by.y="cluster",
              all.x=T, all.y=F)
  
  # Plot 2a
  data2 = df2[infection.source %in% c("Local transmission") & 
              (is.na(foreign.workers)| foreign.workers=="" | 
              (foreign.workers=="y" & !(patient.citizen %in% c("wp", "lp", "sp", "unknown")))),
             list(N=.N),by=c("confirmed.at.date", "patient.citizen")]
  labels$patient.citizen = "citizen"
  data2 = merge(data2, labels[grepl("1|3|4", related)], 
                by.x=c("confirmed.at.date", "patient.citizen"), 
                by.y=c("date", "patient.citizen"), all.x=T, all.y=T)

  data2$patient.citizen = factor(data2$patient.citizen,
      levels=c("citizen", "pr", "ep", "sp", "wp", "lp", "visitor", "unknown"),
      labels=c("Citizen", "PR", "EP", "SP", "WP", "LP", "Visitor", "Unknown"))   

  # data 2b
  data2b = df2[infection.source %in% c("Local transmission") & 
              (is.na(foreign.workers)| foreign.workers=="" | 
              (foreign.workers=="y" & !(patient.citizen %in% c("wp", "lp", "sp", "unknown")))),
             list(N=.N),by=c("confirmed.at.date", "infection.type")][confirmed.at.date <= max_date]
  data2b = dcast(data2b, confirmed.at.date ~ infection.type, value.var="N")
  data2b[is.na(data2b)] = 0
  data2b$prop_unlinked = data2b$`Local unlinked` / (data2b$`Local linked` + data2b$`Local unlinked`) * 100
  data2b = data2b[(data2b$`Local linked` + data2b$`Local unlinked`) > 30]

  link_scale = 1

  plt2 = ggplot(data=data2, aes(x=confirmed.at.date, y=N, fill=patient.citizen, label=no)) +
    geom_bar(position='stack', stat="identity") + 
    ...
    xlab("") + ylab("Number of cases (bars)\n") + 
    ggtitle("Community spread (citizens, PRs, WP/SP/LP not in dorm/construction clusters)") +

    geom_line(data=data2b, 
              aes(x=confirmed.at.date, y=prop_unlinked*link_scale),
              show.legend=F, inherit.aes=F, color=cbp1[2]) +
    ...
    scale_y_continuous(breaks=breaks_f, 
                      sec.axis = sec_axis(~ ./link_scale, name="% of unlinked cases (line)\n",,
                                          breaks=seq(0, 100, 10))) +
    scale_x_date(date_breaks = "week", date_minor_breaks = "1 day", date_labels = "%d-%b",
                 limits=c(min_date-1, max_date+2)) +
    
    geom_label_repel(nudge_y=100, direction="x", angle=90, vjust=0, force=2, 
                     fontface='bold', color="black", size=3, fill="white",
                     segment.size=0.5, segment.color='black', 
                     box.padding = unit(0.4, 'lines'), 
                     arrow = arrow(length = unit(1, "mm")),
                     point.padding = 0, max.iter=3000, show.legend=F)
  ...
  # Plot 3
  data3 = df2[infection.source %in% c("Local transmission") & 
              (foreign.workers=="y" & patient.citizen %in% c("wp", "lp", "sp", "unknown")),
             list(N=.N),by=c("confirmed.at.date")]

  data3 = merge(data3, labels[grepl("5", related)], 
                by.x=c("confirmed.at.date"), 
                by.y=c("date"), all.x=T, all.y=T)

  plt3 = ggplot(data=data3, aes(x=confirmed.at.date, y=N, label=no)) +
    geom_bar(position='stack', stat="identity", fill=cbp1[2]) + 
    
    theme +
    theme(legend.position="bottom", legend.box = "horizontal", 
          axis.text.x=element_text(angle = 90)) + 

    xlab("") + ylab("Number of cases\n(dormitory/ construction related)\n") + 
    ggtitle("Migrant worker-related (in clusters and holding WP/SP/LP)") +
  ...
  # Plots
  grid.arrange(grobs=list(plt1, plt2, plt3), ncol=1)

}, height=1200)
```


On 31 Jan, quarantine measures were issued to all travellers arriving from outbreak areas such as Hubei Province, and this was extended to China as well as other outbreak epicentres such as Italy and Korea on 4 Mar, as well as to Singapore residents from 18 Mar. These aimed at reducing imported cases. Within this time, some local transmission had already occurred, resulting in the seven clusters with 107 cases.

On 7 Feb, Singapore went DORSCON orange. Preparatory (e.g. preparing business continuity plans) and imposed measures were put in place. Imposed measures were largely for social distancing (e.g., encouraging telecommuting to workplaces) and though some also to reduce imported cases (e.g. Singaporeans to defer non-essential travel and foreigners to bear cost of covid19 treatment). 

However a second wave of imported cases started arriving from early March. These were from countries other than China as the contagion had spread outside of China. The largest proportion were from United States and United Kingdom. These imported cases comprised of Singaporeans who were coming back from overseas and certain Singapore Work Pass Holders. On 18 Mar, Malaysia had a border lockdown. On 22 Mar, Singapore closed its borders to visitors but by 1 Apr, there was already a greater proportion of cases with local transmission (i.e. community spread).

During this period, there were also heighted social-distancing measures, especially for vulnerable groups, such as spaced out seating and queuing arrangements in malls, deferring trips to malls, senior activities suspended and no visitors to Old Folks Homes. The defining social-distancing measure was the “circuit breaker” which would last from 7 Apr to 4 May, where many, except those in essential services, in Singapore will work and study from home.

The first wave of imported cases was generally contained. The large influx of second wave imported cases, however, led to sustained local transmission from 1 Apr, especially in the worker dormitories. It appears that imported cases were effectively reduced when border was locked down. However, given that some symptomatic cases would likely had slipped through as the detection of infected cases is not perfect (due to asymptomatic cases and variable incubation time) and the social distancing measures were likely not stringent enough, there was greater community spread from 1 Apr, warranting the "circuit breaker" measure. 

From 11 Apr onwards, workers from these dormitories were actively tested, resulting in the spike of cases. Unless active testing is done (as with the dormitories), the unlinked cases picked up will only be those symptomatic and those of greater severity (not mild cases) which would be the tip of the iceberg...
...
df_deceased_age$cfr = 
  round(df_deceased_age$Deceased / (df_deceased_age$Deceased + df_deceased_age$Recovered)*100, 1)
...
renderPlot({
  ggplot(data=hosp_cases, aes(x=confirmed.at.date, y=cases)) +
    geom_bar(stat="identity", fill=cbp1[3], color=NA, alpha=0.6) +
    facet_wrap(~hospital) +
    scale_y_continuous(breaks=breaks_f) +
    ylab("Number of new cases in hospital\n") + xlab("") + 
    theme
})
```
...
ggplot(df, aes(x=days.to.recover.resources)) + 
  ...
  geom_vline(xintercept=mean_recovery_time, size=.5, col=cbp1[2], linetype="dotted") +
...
renderPlot({
  ...
  gs = list(
    ggplot(data, aes(x=cat, y=days)) + 
      ...
      geom_boxplot(color=cbp1[6], fill=NA, outlier.size = -1) +
      xlab("Age category") + ylab("Days to recover\n") + theme +
      ggtitle("Age"),
  
    ggplot(df[gender %in% c("M", "F")], aes(x=gender, y=days.to.recover.resources)) +
      geom_jitter(height = 0, width = 0.1, color=cbp1[3]) +
      geom_boxplot(color=cbp1[6], fill=NA, outlier.size = -1) +
...
- Pre-symptomatic -> Symptomatic -> Removed
- Asymptomatic -> Removed
- Reinfection?

- Incubation time: 5.5 days
- Recovery time (till 2 -ve tests, on the conservative side): 20 days
...
In the later dataset, EP, SP, WP were separated different work passes. However in the earlier dataset to that all were consider WP but WP/LP were already distinguished.

As of 20 Apr, data on cases and contact tracing information were no longer released by cases. Only recoveries were noted. Up till then, there were 84 days of case-level data from 23 Jan to 19 Apr.
...
# Ideas
# - Prediction of when hospitals will be overwhelmed
# - Contact tracing app
# - Estimating R0 with clusters
```
---
