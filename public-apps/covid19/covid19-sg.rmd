...
```{r}
renderPlot({
  data = df[,list(N=.N),by=c("confirmed.at.date", "patient.citizen")]

  data$patient.citizen = factor(data$patient.citizen,
      levels=c("citizen", "pr", "sp", "wp", "lp", "visitor", "unknown"),
      labels=c("Citizen", "PR", "SP", "WP", "LP", "Visitor", "Unknown"))   

  ggplot(data=data, aes(x=confirmed.at.date, y=N, fill=patient.citizen)) +
    geom_bar(position='stack', stat="identity") + 
    ...
    xlab("") + ylab("Number of cases by status\n") +
