...
gs = list(ggplot(data_1st_wave, aes(x=reorder(camelcase_labeller("", country.of.origin), -N), y=N)) +
  ...
  ggplot(data_2nd_wave, 
    ...
    guides(fill=F) +
    scale_y_continuous(breaks=seq(0, max(data_2nd_wave$N, na.rm=T)+40, 20)) +
...
names(show_deceased_age)[which(names(show_deceased_age)=="category")] = "Category" 
show_deceased_age %>%
...
hosp_cases = df[!(tolower(hospital) %in% c("unknown", "")), list(cases=.N), by=c("confirmed.at.date", "hospital")]
