...
renderPlot({
  ...
  plt1 = ggplot(data=data1, aes(x=confirmed.at.date, y=N)) +
    ...
    annotate("text", x = date_second_wave+2, y=10, label="Second wave", hjust=0) 
  ...
  data2 = df2[infection.source %in% c("Local transmission") & 
              (is.na(foreign.workers)| foreign.workers=="" | (foreign.workers=="y" & !(patient.citizen %in% c("wp", "lp", "sp", "unknown")))),
  ...
  plt2 = ggplot(data=data2, aes(x=confirmed.at.date, y=N, fill=patient.citizen)) +
    geom_bar(position='stack', stat="identity") + 
  ...
  plt3 = ggplot(data=data3, aes(x=confirmed.at.date, y=N)) +
...
Assuming admitting only serious cases (`r prop_admitted20*100`% of active cases), on `r format(show_serious[dim(show_serious)[1]]$date, "%d %b %Y")` with `r show_serious[dim(show_serious)[1]]$case_load` cases, the hospitals are filled by `r show_serious[dim(show_serious)[1]]$proportion20`% and is increasing at a rate of `r show_serious[dim(show_serious)[1]]$proportion20-show_serious[(dim(show_serious)[1]-1)]$proportion20`% (or `r show_serious[dim(show_serious)[1]]$case_load-show_serious[(dim(show_serious)[1]-1)]$case_load` cases with `r round((show_serious[dim(show_serious)[1]]$case_load-show_serious[(dim(show_serious)[1]-1)]$case_load)*prop_admitted20, 0)` serious cases) from yesterday.
