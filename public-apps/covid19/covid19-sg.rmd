...
data_1st_wave = data_1st_wave[, list(N=length(case)),by="patient.nationality"]
data_1st_wave[patient.nationality==""]$patient.nationality = "singaporean"

data_2nd_wave = data.table(df[df$confirmed.at.date >= date_second_wave &
                              df$infection.source == "imported case",])
data_2nd_wave = data_2nd_wave[, list(N=length(case)),by="patient.nationality"]
data_2nd_wave[patient.nationality==""]$patient.nationality = "singaporean"

grid.arrange(
  ggplot(data_1st_wave, aes(x=camelcase_labeller("", patient.nationality), y=N)) +
  geom_bar(stat = "identity") + coord_flip() + theme_minimal() +
  xlab("") + ylab("") + ggtitle("First wave"),
  ggplot(data_2nd_wave, aes(x=camelcase_labeller("", patient.nationality), y=N)) +
  geom_bar(stat = "identity") + coord_flip() + theme_minimal() +
...
Here we attempt to investigate if there is an association with number of days to recover and age. Age does not appear to be associated with the number of days to recover, though [fatalities are higher for elderly above 65](https://www.businessinsider.sg/most-us-coronavirus-deaths-ages-65-older-cdc-report-2020-3).
