...
renderPlot({
  ...
  geom_rect(data = data.frame(x = 0, y = 0), 
           aes(xmin=min(data$confirmed.at.date-add_date), xmax=max(data$confirmed.at.date)+add_date, 
               ymin=0, ymax=Inf), 
           fill="skyblue", show.legend=F, alpha=0.2) +
  geom_rect(data = data.frame(x = 0, y = 0), 
            aes(xmin=min(data$confirmed.at.date-add_date), xmax=max(data$confirmed.at.date)+add_date, 
                ymin=-Inf, ymax=0), 
            fill="#dbffbf", show.legend=F, alpha=0.2) +
