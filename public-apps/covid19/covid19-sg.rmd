...
clusters = cluster_cases[, list(cases=.N,
                                ...
                                max.date=max(confirmed.at.date)), by=cluster][order(-cases)][1:10]
...
ggplot(cluster_cases[cluster %in% clusters$cluster], aes(x=reorder(cluster.short.no, -no))) +
...
show_clusters = clusters[, c("no", "cluster", "type", "cases", "proportion", 
                             "min.date", "max.date", "duration")][order(no)]
...
```{r}
cluster_cases$no = factor(cluster_cases$no, levels=sort(as.numeric(unique(cluster_cases$no))))
ggplot(cluster_cases[cluster %in% clusters$cluster], aes(x=age)) +
  geom_histogram(binwidth=10) + theme +
  facet_wrap(~no, ncol=3, scales="free_y") + 
