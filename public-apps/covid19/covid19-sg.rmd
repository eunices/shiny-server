...
ggplot(df, aes(x=days.to.recover.symptomatic)) + 
  geom_histogram(binwidth=binwidth, na.rm=T, color=cbp1[3], fill=cbp1[6]) + 
...
renderPlot({
  ...
  gs = list(
    ...
    ggplot(df[gender != "unknown"], aes(x=gender, y=days.to.recover.symptomatic)) +
      geom_boxplot(color=cbp1[6], fill=NA) +
      geom_jitter(height = 0, width = 0.1, color=cbp1[3]) +
      xlab("Gender") + ylab("Days to recover\n") + theme +
      ggtitle("Gender")
  )
...
names(clusters) = unlist(lapply(gsub("\\.", " ", names(clusters)), 
                                function(x) camelcase_labeller("", x)))
knitr::kable(clusters, "markdown")
```
