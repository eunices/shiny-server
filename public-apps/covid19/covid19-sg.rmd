...
renderPlot({
  ...
  data2 = df2[infection.source %in% c("Local transmission") & 
              (foreign.workers=="" | (foreign.workers=="y" & !(patient.citizen %in% c("wp", "lp", "sp", "unknown")))),
  ...
  # Plot 4
  data4 = df2[infection.source %in% c("unknown"),
             list(N=.N),by=c("confirmed.at.date")]

  plt4 = ggplot(data=data4, aes(x=confirmed.at.date, y=N)) +
    geom_bar(position='stack', stat="identity", fill=cbp1[1]) + 
  ...
  # Plots
  grid.arrange(grobs=list(plt1, plt2, plt3, plt4), ncol=1)
}, height=1200)
```
...
As of 17 Apr 2020, the hospitals seem to be filling up fast, and Singapore would probably need to change its strategy soon, not to admit [milder Covid-19 cases](https://www.straitstimes.com/singapore/health/coronavirus-experts-say-big-jump-in-infections-is-stretching-the-healthcare-system).
