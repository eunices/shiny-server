...
```{r}
mean_hospitalisation_rate = mean((df_cases_day[date>=date_no_hosp]$I2 + df_cases_day[date>=date_no_hosp]$I1)/ df_cases_day[date>=date_no_hosp]$case_load * 100, na.rm=T)
```

- Mean percentage of cases in hospital care (from `r format(date_no_hosp, "%d %b %Y")`): `r  round(mean_hospitalisation_rate, 1)`%
...
```{r}
renderPlot({
  sec_axis_name = "% of maximum patient load\n"
  ggplot(data=df_cases_day, aes(x=date, y=I2)) +
    ...
    xlab("") + ylab("Number of hospitalised persons\n") + 
    scale_x_date(date_breaks = "week", date_minor_breaks = "1 day", date_labels = "%d-%b") +
...
renderPlot({
  ggplot(df[!tolower(hospital) %in% c("unknown", "not admitted"), 
    ...
    geom_bar(stat="identity", color=cbp1[6]) +
    facet_wrap(~hospital) +
    ylab("Number of new cases\n") + xlab("") + ggtitle("New cases conveyed to hospitals")
    theme
})
```
...
renderPlot({
  ggplot(melted_df_cases_day_hosp[hospital !="" & variable=="I"], aes(x=date, y=value)) +
    geom_line(show.legend=F) +
    facet_wrap(~hospital) +
    ylab("Number of active cases in hospital\n") + xlab("") + ggtitle("Active cases in hospitals") +
    theme
})
```
...
renderPlot({
  cluster_cases$patient.citizen = factor(cluster_cases$patient.citizen,
    levels=c("citizen", "pr", "wp", "lp", "visitor", "unknown"),
    labels=c("Citizen", "PR", "WP", "LP", "Visitor", "Unknown"))
  ggplot(cluster_cases[no %in% 1:10], aes(x=patient.citizen, fill=patient.citizen)) +
    ...
    scale_y_continuous(breaks=breaks_f) +
    scale_fill_manual(name="", values=cbp1[c(2, 7, 3, 6, 4, 1)]) +
