...
renderPlot({
  ...
  # Plot 2a
  data2a = df2[infection.source %in% c("Local transmission") & 
  ...
  data2a = merge(data2a, labels[grepl("1|3|4", related)], 
                by.x=c("confirmed.at.date", "patient.citizen"), 
                by.y=c("date", "patient.citizen"), all.x=T, all.y=T)

  data2a$patient.citizen = factor(data2a$patient.citizen,
      levels=c("citizen", "pr", "ep", "sp", "wp", "lp", "visitor", "unknown"),
      labels=c("Citizen", "PR", "EP", "SP", "WP", "LP", "Visitor", "Unknown"))   

  plt2a = ggplot(data=data2a, aes(x=confirmed.at.date, y=N, fill=patient.citizen, label=no)) +
    geom_bar(position='stack', stat="identity") + 
  ...
  data2b = df2[infection.source %in% c("Local transmission") & 
              (is.na(foreign.workers)| foreign.workers=="" | 
              (foreign.workers=="y" & !(patient.citizen %in% c("wp", "lp", "sp", "unknown")))),
             list(N=.N),by=c("confirmed.at.date", "infection.type")]
  labels$infection.type = "Local linked"
  data2b = merge(data2b, labels[grepl("1|3|4", related)], 
                by.x=c("confirmed.at.date", "infection.type"), 
                by.y=c("date", "infection.type"), all.x=T, all.y=T)

  data2b$patient.citizen = factor(data2b$patient.citizen,
      levels=c("citizen", "pr", "ep", "sp", "wp", "lp", "visitor", "unknown"),
      labels=c("Citizen", "PR", "EP", "SP", "WP", "LP", "Visitor", "Unknown"))   

  plt2b = ggplot(data=data2b, aes(x=confirmed.at.date, y=N, fill=infection.type, label=no)) +
    geom_bar(position='stack', stat="identity") + 
  ...
  # Plots
  grid.arrange(grobs=list(plt1, plt2a, plt2b, plt3), ncol=1)
}, height=1200)
```
