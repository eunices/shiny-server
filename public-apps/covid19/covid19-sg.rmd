...
renderPlot({
  ...
  plt = ggplot(data=data[confirmed.at.date >= min_date & confirmed.at.date <= max_date], 
    ...
    scale_fill_manual(name="", 
                      labels=c("Recovered", "Event", "Event/measure",
                              "Imported case", "Local linked", "Local unlinked",
                              "Measure", "Unknown"),
                      values=c(cbp1[c(4, 5, 2, 6, 3)], "#8ccff5", "tomato1", cbp1[1])) + 
    theme +
...
mean_icu_rate = mean((df_cases_day[date>=date_no_hosp]$I1)/ df_cases_day[date>=date_no_hosp]$case_load * 100, na.rm=T)
```
...
renderPlot({
  ggplot(data=df_cases_day, aes(x=date, y=cfr)) + 
    ...
    scale_y_continuous(sec.axis=sec_axis(~ ./death_scale, name="Cumulative deaths (light orange)\n",
                                         breaks=breaks_f, minor_breaks=breaks_f)) +
