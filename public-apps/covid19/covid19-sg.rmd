...
> **DISCLAIMER: I am not an epidemiologist, so the information below should not be used for policy. Neither has this document gone through peer-review, so please do not cite it.**
...
Note: cases whose hospitals were pending at time of press release and thos who are not admitted are omitted from table above.
...
renderPlot({
  ...
  gs = list(
    ...
    ggplot(df[gender != "unknown"], aes(x=gender, y=days.to.recover.symptomatic)) +
      geom_boxplot(color=cbp1[6], fill=NA) +
      geom_jitter(height = 0, width = 0.1, color=cbp1[3]) +
...
```{r}
clusters = df[cluster != "", list(cases=.N), by=cluster][order(-cases)][1:10]
...
clusters = clusters[, c("no", "cluster", "cases", "proportion", "cluster.short")]
```

```{r fig.width=10, fig.height=3}
df = merge(df, clusters[, c("cluster", "no", "cluster.short")], 
           by.x="cluster", by.y="cluster", all.x=T, all.y=F)
df$cluster.short = paste0(df$cluster.short, " [", df$no, "]")
ggplot(df[cluster %in% clusters$cluster], aes(x=reorder(cluster.short, -no))) +
...
```{r}
show_clusters = clusters[, c("no", "cluster", "cases", "proportion")]
...
```{r}
# TODO: duration of cluster (+ earliest and latest case)
# TODO: progression of cluster with time (facet plot)
# TODO: type of venue of cluster/ map of cluster (ggmap)
```
