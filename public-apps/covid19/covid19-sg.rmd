...
lab_cols = data.frame(label=c("Event", "Event/measure",
                              "Imported case", "Local transmission", 
                              "Measure", "Unknown"),
                      colour=c(cbp1[c(5, 2, 6, 3)], "tomato1", cbp1[1]))
...
renderPlot({
  ...
  plt = ggplot(data=data[confirmed.at.date >= min_date & confirmed.at.date <= max_date], 
    ...
    scale_fill_manual(name="", 
                      labels=lab_cols$label,
                      values=lab_cols$colour) + 
    theme +
...
ggplot(df, aes(x=days.to.recover.resources)) + 
  ...
  xlab("\nDays to recover") + ylab("Number of cases\n") +
