...
The first case detected in Singapore on `r format(min_date, "%d %B %Y")`. Key measures and events are marked below.
...
renderPlot({
  ...
  labels$yend = data[confirmed.at.date %in% labels$x, list(sum=sum(N)), by="confirmed.at.date"]$sum
  ...
  plt = ggplot(data=data, aes(x=confirmed.at.date, y=N, fill=infection.source, label=short)) +
  ...
  plt = plt + geom_label_repel(nudge_y=100, angle=0, vjust=0, force=5,
                               fontface='bold', color='black', size=3, 
                               segment.size=0.5, segment.color='black', 
                               box.padding = unit(0.25, 'lines'), fill='grey97',
...
```{r}
# **Events**
# show_events = events[grepl("event", type)][,c("date", "short")]
# names(show_events) = c("Date", "Event")
# knitr::kable(show_events, "markdown")
```
...
show_measures$event = paste0(show_measures$short, " (", format(show_measures$date, "%d %b"), ")")
...
knitr::kable(show_measures, "markdown")
```
...
renderWordcloud2({wordcloud2(data=data_2nd_wave, size=3, 
                             color=rep_len(cbp1[c(6, 3, 1)], nrow(data_2nd_wave)))}) # bug
