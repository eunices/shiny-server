...
observeEvent(input$add, { 
    ...
    values$pi_sim_li = c(values$pi_sim_li, round(values$pi_sim, 7))
    if(length(values$pi_sim_li) > 5) values$pi_sim_li <- values$pi_sim_li[-1] # remove 1st
})
    ...
    fluidRow(
        ...
        column(6,
            fluidPage(
                ...
                renderText({
                    len = length(values$pi_sim_li)
                    if(len>0) {
                        paste("Previous values: ", paste(values$pi_sim_li[-len], collapse=", "))
                    } else {
                        paste0("No previous values.")
                    }
                }),
                renderText({ 
