...
observeEvent(input$add, { 
    values$step_size = ifelse(values$counter <= 1000, per_step, per_step *10)
    values$counter = values$counter + step_value
    x = runif(step_value, min=min, max=max)
    y = runif(step_value, min=min, max=max)
    ...
    fluidRow(
        ...
        column(6,
            fluidPage(
                ...
                renderText({ 
                    paste("Currently: ", values$step_size, " points added")
                }),
                renderText({
